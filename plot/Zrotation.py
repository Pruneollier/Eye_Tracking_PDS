...
def plot_rotations_and_diffs(csv_file):
    ...
    df['Zrot_normalized'] = (df['Zrot'] - df['Zrot'].min()) / (df['Zrot'].max() - df['Zrot'].min())
    ...
    # Plot Xrot, Zrot, Zrot over Timestamp
    axs[0].plot(df['Timestamp'], df['Zrot_normalized'], label='Zrot', color='orange')
    axs[0].plot(df['Timestamp'], df['Xdiff_normalized'], label='Xdiff', color='red')
    axs[0].set_ylabel('Zrot and Xdiff, normalized')
    axs[0].set_xlabel('time')
    axs[0].set_title('Influence of Zrot over Xdiff over time')
    axs[0].legend()



    axs[1].plot(df['Timestamp'], df['Zrot_normalized'], label='Zrot', color='orange')
    axs[1].plot(df['Timestamp'], df['Ydiff_normalized'], label='Ydiff', color='red')
    axs[1].set_ylabel('Zrot and Ydiff, normalized')
    axs[1].set_xlabel('time')
    axs[1].set_title('Influence of Zrot over Ydiff over time')
    axs[1].legend()
