...
def plot_rotations_and_diffs(csv_file):
    ...
    df['Yface_normalized'] = (df['Yface'] - df['Yface'].min()) / (df['Yface'].max() - df['Yface'].min())
    ...
    # Plot Yface, Yface, Yface over Timestamp
    axs[0].plot(df['Timestamp'], df['Yface_normalized'], label='Yface', color='orange')
    axs[0].plot(df['Timestamp'], df['Xdiff_normalized'], label='Xdiff', color='red')
    axs[0].set_ylabel('Yface and Xdiff, normalized')
    axs[0].set_xlabel('time')
    axs[0].set_title('Influence of Yface over Xdiff over time')
    axs[0].legend()



    axs[1].plot(df['Timestamp'], df['Yface_normalized'], label='Yface', color='orange')
    axs[1].plot(df['Timestamp'], df['Ydiff_normalized'], label='Ydiff', color='red')
    axs[1].set_ylabel('Yface and Ydiff, normalized')
    axs[1].set_xlabel('time')
    axs[1].set_title('Influence of Yface over Ydiff over time')
    axs[1].legend()
