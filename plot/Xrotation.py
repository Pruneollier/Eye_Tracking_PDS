...
def plot_rotations_and_diffs(csv_file):
    ...
    df['Xrot_normalized'] = (df['Xrot'] - df['Xrot'].min()) / (df['Xrot'].max() - df['Xrot'].min())
    ...
    # Plot Xrot, Xrot, Xrot over Timestamp
    axs[0].plot(df['Timestamp'], df['Xrot_normalized'], label='Xrot', color='orange')
    axs[0].plot(df['Timestamp'], df['Xdiff_normalized'], label='Xdiff', color='red')
    axs[0].set_ylabel('Xrot and Xdiff, normalized')
    axs[0].set_xlabel('time')
    axs[0].set_title('Influence of Xrot over Xdiff over time')
    axs[0].legend()



    axs[1].plot(df['Timestamp'], df['Xrot_normalized'], label='Xrot', color='orange')
    axs[1].plot(df['Timestamp'], df['Ydiff_normalized'], label='Ydiff', color='red')
    axs[1].set_ylabel('Xrot and Ydiff, normalized')
    axs[1].set_xlabel('time')
    axs[1].set_title('Influence of Xrot over Ydiff over time')
    axs[1].legend()
