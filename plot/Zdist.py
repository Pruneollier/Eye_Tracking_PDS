...
def plot_rotations_and_diffs(csv_file):
    ...
    df['Zface_normalized'] = (df['Zface'] - df['Zface'].min()) / (df['Zface'].max() - df['Zface'].min())
    ...
    # Plot Zface, Zface, Zface over Timestamp
    axs[0].plot(df['Timestamp'], df['Zface_normalized'], label='Zface', color='orange')
    axs[0].plot(df['Timestamp'], df['Xdiff_normalized'], label='Xdiff', color='red')
    axs[0].set_ylabel('Zface and Xdiff, normalized')
    axs[0].set_xlabel('time')
    axs[0].set_title('Influence of Zface over Xdiff over time')
    axs[0].legend()



    axs[1].plot(df['Timestamp'], df['Zface_normalized'], label='Zface', color='orange')
    axs[1].plot(df['Timestamp'], df['Ydiff_normalized'], label='Ydiff', color='red')
    axs[1].set_ylabel('Zface and Ydiff, normalized')
    axs[1].set_xlabel('time')
    axs[1].set_title('Influence of Zface over Ydiff over time')
    axs[1].legend()
