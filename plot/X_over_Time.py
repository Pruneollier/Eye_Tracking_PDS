...
first_non_zero_timestamp = df2.loc[df2['X'] != 0, 'Timestamp'].min()
...
# Plotting
plt.plot(df1['Timestamp'], df1['X'], label='X coordinate of gaze estimation indicator')
plt.plot(df2['Timestamp'], df2['X'], label='X coordinate of touch')

# Customize the plot
plt.xlabel('Time (mm:ss)')
plt.ylabel('X (CGFloat)')
plt.title('Comparison of X coordinates over time')
plt.legend()
...
# Calculate the x-coordinate difference at each timestamp
x_diff = abs(df1['X'] - df2['X'])
